...
- For `lookup`, use `topic="news"` for queries containing "tin", "tin tức", or "news". Map date references when explicitly present in the query: "hôm nay"/"today" -> `timeframe="day"`, "tuần này"/"this week" -> `timeframe="week"`, "tháng này"/"this month" -> `timeframe="month"`, "năm nay"/"this year" -> `timeframe="year"`. If no news or date reference is in the query, do NOT set non-default values for `topic` or `timeframe`.
